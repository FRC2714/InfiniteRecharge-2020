    ...
    public SideTrenchRun(Drivetrain drivetrain, Intake intake, Conveyor conveyor, Shooter shooter, Limelight limelight){
        CustomRamseteCommand quinticStraightLineToTrench =
                RamseteGenerator.getRamseteCommand(
                        drivetrain,
                        List.of(
                                new Pose2d(Units.feetToMeters(11.89), Units.feetToMeters(24.65), new Rotation2d().fromDegrees(0.34)),
        ...
        addCommands(
                sequence(
                        ...
                        new AutomaticShooter(shooter,conveyor,intake, 2000, true,3).withTimeout(4),
                        deadline(
                                quinticPickupBalls,
                                new AutoIntake(shooter,intake,conveyor, AutoIntake.IntakeType.INTAKE)
                        ),
                        new AlignToTarget(drivetrain, limelight),
                        new AutomaticShooter(shooter,conveyor,intake, 2300, false,3).withTimeout(5)
                )
        );
    }
